...
def batch_predict_(pc, window_size, stride_size, model_name, label_type, device, model):
    pred_labels = []  # For each sequence, store the true category and model prediction category of the segmented patc
    patch_dataset = get_testing_patches(pc, window_size, stride_size)
    test_dataset = MyDataset_test(dataset=patch_dataset, name=label_type, transform=None)
    test_loader = DataLoader(dataset=test_dataset, batch_size=1, shuffle=False, num_workers=0)
...
class GUI_Main(QMainWindow, Ui_MainWindow):
    ...
    def initUI(self):
        ...
        self.pushButton_3.clicked.connect(self.explaintion)  # 扰动解释
    ...
    def explaintion(self):
        ...
        loc_pre = batch_predict_(pc, window_size, stride_size, model_name, label_type, device, model)
        ...
        explanation = explainer.explain_instance(pc,
                                                 ...
                                                 classifier_fn=batch_predict_,  # the predicted function
        ...
        mask, metrics = explanation.get_weight_and_shap(1, method)  # 0-KT， 1-PD。表示感兴趣的类别
        ...
        percentages = np.arange(0.1, 1.1, 0.1)  # from 10% to 100%
        positive_values = []
        negative_values = []
        _, fudged_data = segment_fn(pc)  # 存储的是每个片段的均值，与原始数据相同尺寸
        ...
        # plt.ylim(-1, 1)  # 设置Y轴取值范围为[-1, 1]
        plt.grid(True)
        plt.xticks(fontsize=20)  # X轴标签的字体大小
